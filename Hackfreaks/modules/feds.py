...
import Hackfreaks.modules.sql.feds_sql as sql
...
from Hackfreaks.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "hackfreaks_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="hackfreaks_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "hackfreaks_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="hackfreaks_fbanned_users.csv",
